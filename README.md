...
It first sets the shortest distance as the players row – the monster's row and the player column – the monsters column. It then checks the spaces for possible moves ensuring it is both within the bounds of the map and that it isn't a monster or wall or coin etc. It calculates all the paths distances and if the route it has found is the shortest current route, then it updates the shortest route and sets the best move as the adjacent space in that path. If the bestMove is not the monster's current position, then the current position of the monster will be changed to a ”.” and the position of bestMove will be changed to a ”O” and the monster position will be updated. </br>
